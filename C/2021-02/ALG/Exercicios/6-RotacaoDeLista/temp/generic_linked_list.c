...
void append_to_list(list_t *l, void *datum) {
    ...
    p->datum = malloc(l->elem_size);
    memcpy(p->datum, datum, l->elem_size);
...
void access_datum(list_t *l, int index, void *datum) {
    ...
    memcpy(datum, curr_node_pointer->datum, l->elem_size);
}
...
void right_rotate_list(list_t *l, int n) {
    ...
    for (int i = 0; i < n; i++) {
        access_datum(l, last_index, datum);
